...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if(user.pro === false && user.todos.length >= 10){
    return response.status(403).json({ error: "User cannot create todos anymore" })
  }
  return next()
}
...
function checksTodoExists(request, response, next) {
  ...
  const uuidChecked = validator.isUUID(id, [4])
...
function findUserById(request, response, next) {
  const { id } = request.params
  const idChecked = users.find(user => user.id === id)
  if(!idChecked){
    return response.status(404).json({ error: "User ID not found" })
  }
  request.user = idChecked
  return next()
}
